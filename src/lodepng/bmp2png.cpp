...
int convert_png(string filename)
{
	...
	if (error)
	{
		guru::nonfatal("Could not decode BMP file!", GURU_ERROR);
		return 0;
	}
	...
	if (error)
	{
		guru::nonfatal("Could not encode PNG file!", GURU_ERROR);
		return 0;
	}
	temp.swap(png);
	lodepng::save_file(png, dest.c_str());
	error = remove(source.c_str());
	if (error) guru::nonfatal("Could not delete file: " + source, GURU_WARN);
	return 0;
}
