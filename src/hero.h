...
{
public:
			Hero(unsigned long long new_id);
			~Hero();
	shared_ptr<Controls>	controls();	// Returns a pointer to this Hero's Controls AI.
	void	load(unsigned long long owner_id = 0) override;	// Loads the Hero's data from disk.
	...
	void	save(unsigned long long owner_id = 0) override;	// Saves the Hero's data to disk, along with the rest of the game world.
